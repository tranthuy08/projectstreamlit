...
# Kiểm tra các giá trị trong cột genre và budget
st.write(movies_data['genre'].unique())  # Kiểm tra các thể loại
st.write(movies_data['budget'].describe())  # Kiểm tra các thống kê của ngân sách

# Lọc theo thể loại
genre_selected = st.selectbox("Chọn thể loại phim:", options=movies_data['genre'].unique())
st.write(f"**Danh sách phim thuộc thể loại: {genre_selected}:**")

# Lọc theo năm
year_range = st.slider("Chọn khoảng thời gian phim (năm):", int(movies_data['year'].min()), int(movies_data['year'].max()), (int(movies_data['year'].min()), int(movies_data['year'].max())))
filtered_data = movies_data[(movies_data['genre'] == genre_selected) & (movies_data['year'] >= year_range[0]) & (movies_data['year'] <= year_range[1])]
...
rating_categories = movies_data['rating'].astype('category')
movies_data['rating_code'] = rating_categories.cat.codes
# Vẽ biểu đồ scatter 
